xs.define(xs.Class, 'ns.tests.xs.pointer.stage.Enter', function (self, imports) {
    ...
    Class.imports = {
        event: {
            Enter: 'xs.view.event.pointer.Enter'
        }
    };

    Class.extends = 'ns.module.test.Stage';

    Class.constant.instruction = 'enter block for 3 times for different places';
    ...
    Class.method.start = function () {
        ...
        block.on(imports.event.Enter, countdownHandler);

        me.private.cleanUp = function () {
            block.off(imports.event.Enter);
        };
    };
