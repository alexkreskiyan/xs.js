xs.define(xs.Class, 'ns.tests.xs.pointer.stage.Leave', function (self, imports) {
    ...
    Class.imports = {
        event: {
            Leave: 'xs.view.event.pointer.Leave'
        }
    };

    Class.extends = 'ns.module.test.Stage';

    Class.constant.instruction = 'leave block for 3 times for different places';
    ...
    Class.method.start = function () {
        ...
        block.on(imports.event.Leave, countdownHandler);

        me.private.cleanUp = function () {
            block.off(imports.event.Leave);
        };
    };
    ...
    Class.method.stop = function () {
        ...
        me.private.container.sandbox.at(0).off(imports.event.Leave);
