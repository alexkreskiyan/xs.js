xs.define(xs.Class, 'ns.tests.xs.element.stage.Focus', function (self, imports) {
    ...
    Class.imports = {
        event: {
            Focus: 'xs.view.event.element.Focus'
        }
    };

    Class.extends = 'ns.module.test.Stage';

    Class.constant.instruction = 'make input gain focus 2 times';
    ...
    Class.method.start = function () {
        ...
        input.on(imports.event.Focus, countdownHandler);
    };

    Class.method.stop = function () {
        var me = this;

        me.private.container.sandbox.at(0).off(imports.event.Focus);
