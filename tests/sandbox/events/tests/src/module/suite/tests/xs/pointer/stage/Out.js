xs.define(xs.Class, 'ns.tests.xs.pointer.stage.Out', function (self, imports) {
    ...
    Class.imports = {
        event: {
            Out: 'xs.view.event.pointer.Out'
        }
    };

    Class.extends = 'ns.module.test.Stage';

    Class.constant.instruction = 'move out of red block for 3 times for different places';
    ...
    Class.method.start = function () {
        ...
        block.on(imports.event.Out, countdownHandler);

        me.private.cleanUp = function () {
            block.off(imports.event.Out);
        };
    };
    ...
    Class.method.stop = function () {
        ...
        me.private.container.sandbox.at(0).off(imports.event.Out);
