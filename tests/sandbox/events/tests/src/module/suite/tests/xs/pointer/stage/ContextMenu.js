xs.define(xs.Class, 'ns.tests.xs.pointer.stage.ContextMenu', function (self, imports) {
    ...
    Class.imports = {
        event: {
            ContextMenu: 'xs.view.event.pointer.ContextMenu'
        }
    };

    Class.extends = 'ns.module.test.Stage';

    Class.constant.instruction = 'call context menu anywhere in sandbox 10 times in different places';
    ...
    Class.method.start = function () {
        ...
        sandbox.on(imports.event.ContextMenu, countdownHandler);

        me.private.cleanUp = function () {
            sandbox.off(imports.event.ContextMenu);
        };
    };
    ...
    Class.method.stop = function () {
        ...
        me.private.container.query('.sandbox').off(imports.event.ContextMenu);
    };
